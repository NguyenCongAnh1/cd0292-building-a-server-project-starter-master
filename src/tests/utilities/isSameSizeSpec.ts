...
describe('isSameSize function', () => {
    it('should return true for an image with the same size', async () => {
      ...
      spyOn(sharp.prototype, 'metadata').and.returnValue(Promise.resolve({ width: expectedWidth, height: expectedHeight }));
    ...
    it('should return false for an image with different size', async () => {
        ...
        spyOn(sharp.prototype, 'metadata').and.returnValue(Promise.resolve({ width: expectedWidth + 1, height: expectedHeight }));
